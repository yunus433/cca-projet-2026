...
void benchmark(
  ...
  fmpq_mpoly_t P, Q, R_lib, R_inter;
  ...
  fmpq_mpoly_init(R_lib, ctx);
  fmpq_mpoly_init(R_inter, ctx);
  ...
  for (int expo_bit_size = expo_bit_size_start; expo_bit_size <= expo_bit_size_end; expo_bit_size++) {
    ...
    sprintf(output_name, "fmpq_expo_bit_size_%d", expo_bit_size);
    ...
    for (int length = length_start; length <= length_end; length += length_increment) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        start = clock();
        if (!fmpq_mpoly_resultant(R_lib, P, Q, var_to_compute, ctx)) {
        ...
        // const char *vars[] = {"x", "y"};
        // fmpq_mpoly_print_pretty(R_lib, vars, ctx);
        // printf("\n");
        ...
        start = clock();
        if (fmpq_mpoly_resultant_interpolation_mode(R_inter, P, Q, var_to_compute, ctx, 0, SMALL_ORDERED)) {
          printf("ERROR: fmpq_mpoly_resultant_interpolation function call returned non-zero error code.\n");
          goto cleanup;
        }
        end = clock();
        ...
        start = clock();
        if (fmpq_mpoly_resultant_interpolation_mode(R_inter, P, Q, var_to_compute, ctx, 0, SMALL_ORDERED_POS)) {
          printf("ERROR: fmpq_mpoly_resultant_interpolation function call returned non-zero error code.\n");
          goto cleanup;
        }
        end = clock();
...
cleanup:
  ...
  fmpq_mpoly_clear(R_lib, ctx);
  fmpq_mpoly_clear(R_inter, ctx);
