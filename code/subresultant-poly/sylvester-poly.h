...
int fmpz_poly_sylvester_mat_poly(
  fmpz_poly_mat_t S, // Not a pointer, already accessed by referenced in Sylvester types. No dynamic memory allocation either, so no need to free later on
  const fmpz_poly_t P,
  const fmpz_poly_t Q
) {
