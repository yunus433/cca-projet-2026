...
To compute $\gcd(f,g)$, a common approach is to use the Euclidean algorithm, which consists of computing the sequence of remainders $r_0 = f$, $r_1 = g$, and $r_{i+1} = r_{i-1} \mod r_i$ until we reach a remainder that is zero. The last non-zero remainder is then the greatest common divisor of $f$ and $g$. However, this algorithm can be inefficient, as the size of the coefficients of the remainders can grow rapidly, leading to a large number of bit operations.
...
In this report, we design and implement a fast algorithm for computing the resultant and subresultant sequence of two multivariate polynomials over $\mathbb{Q}$, $\mathbb{Z}$, or $\mathbb{F}_p$. The approach leverages the specialization property of subresultant polynomials and can be viewed as a generalization of Collins’s algorithm for computing resultants.
Our implementation in FLINT demonstrates significant performance improvements over existing approaches in Wolfram, Maple, and FLINT itself, achieving speedups of up to 7x for the resultant of bivariate polynomials with exponents of bit size $6$ and coefficients of bit size $50$. We discuss the algorithmic design, implementation details, performance benchmarks, and potential avenues for further optimization in the subsequent sections of this report.

All implementations, as well as this research paper and findings are available in the following GitHub repository $\href{https://github.com/yunus433/cca-projet-2026/}{github.com/yunus433/cca-projet-2026}$.